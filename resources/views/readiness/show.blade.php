<x-app-layout>
    ...
    @php
        ...
        $readinessPct = max(0, min(100, $totalReadiness));

        $ringOffset = $circumference * (1 - $readinessPct / 100);
        ...
        <div class="max-w-5xl mx-auto px-4 sm:px-6 space-y-6">
            ...
            <div
                ...
                <div class="grid grid-cols-1 md:grid-cols-12 gap-6 items-center">
                    ...
                    <div
                        class="md:col-span-5 flex flex-col items-center text-center border-b md:border-b-0 md:border-r border-primary/15 pb-6 md:pb-0 md:pr-6">
                        <span
                        ...
                        <div class="relative my-4" style="width: 160px; height: 160px;" x-data="readinessRing(
                            @js($readinessPct),
                            @js(round($circumference, 2))
                        )">

                            <svg style="width: 160px; height: 160px;" class="-rotate-90 transform"
                                viewBox="0 0 160 160">
                                {{-- Background Track --}}
                                ...
                                <circle x-ref="ring" cx="80" cy="80" r="{{ $radius }}"
                                    fill="none" stroke="{{ $bandConfig['stroke'] }}" stroke-width="14"
                                    stroke-linecap="round" stroke-dasharray="{{ round($circumference, 2) }}"
                            ...
                            <div class="absolute inset-0 flex flex-col items-center justify-center">
                                <span class="font-display text-4xl font-extrabold text-slate-900 leading-none">
                                    <span x-text="displayScore">
                                        {{ number_format($readinessPct, 1) }}
                                    </span>
                                </span>
                                <span
                                    class="font-display text-[10px] font-bold uppercase tracking-wider text-muted-ink mt-1">
                                    Weighted TOS
                                </span>
                            </div>
                        </div>
                        ...
                        <span
                            class="inline-flex items-center rounded-full border px-3.5 py-1 font-display text-xs font-bold shadow-sm {{ $bandConfig['bg'] }} {{ $bandConfig['text'] }} {{ $bandConfig['border'] }}">
                            {{ $bandConfig['label'] }}
                        </span>
                    </div>
                    ...
                    <div class="md:col-span-7 space-y-4">
                        <div>
                            ...
                            <p class="mt-1 text-xs leading-relaxed text-slate-700">
                                Calculated from your Bayesian Knowledge Tracing (BKT) competencies across each Table of
                                Specifications area, weighted according to the official PRC blueprint.
                            </p>
                        </div>
                        ...
                        <div class="grid grid-cols-2 sm:grid-cols-3 gap-3 pt-1">
                            ...
                            <div
                                ...
                                <span
                                    class="font-display text-lg font-bold {{ $pointsNeeded > 0 ? 'text-weak-ink' : 'text-strong-ink' }}">
                        ...
                        @if ($weakestDomain)
                            <div
                                class="rounded-xl border border-clinical/30 bg-white/85 p-3.5 text-xs text-clinical-ink leading-relaxed shadow-sm">
                                <span class="font-bold">Remediation Priority:</span>
                                Your lowest mastery is in <strong>{{ $weakestDomain['domain_name'] }}</strong>
                                ({{ number_format(($weakestDomain['mastery'] ?? 0) * 100, 1) }}%). Prioritizing
                                practice questions here provides the fastest path toward closing your readiness gap.
                            </div>
                        @endif
                    </div>
                    ...
                    @foreach ($breakdown['domains'] as $domain)
                        @php
                            ...
                            $badgeColor = match (true) {
                                $domainPct >= $boardReadyThreshold
                                    => 'border-[#22C55E]/40 bg-strong-tint text-strong-ink',
                        ...
                        <div class="rounded-2xl border-2 border-b-4 border-slate-200 bg-white p-5 shadow-sm space-y-4">
                            ...
                            <div>
                                <div class="flex items-start justify-between gap-2">
                                    ...
                                    <span
                                        class="rounded-full border px-2 py-0.5 font-display text-[11px] font-bold {{ $badgeColor }} shrink-0">
                                ...
                                <div class="mt-2.5 h-2.5 w-full overflow-hidden rounded-full bg-slate-100 ring-1 ring-inset ring-slate-200/60"
                                    x-data="{ currentWidth: '0%' }" x-init="$nextTick(() => { setTimeout(() => currentWidth = '{{ min(100, max(0, round($domainPct, 1))) }}%', 120) })">
                                    <div class="h-full rounded-full {{ $barColor }} transition-all duration-1000 ease-out"
                                        style="width: 0%;" :style="'width: ' + currentWidth"></div>
                                </div>

                                {{-- PRC Weight & Contribution Strip --}}
                                <div
                                    class="mt-2.5 flex items-center justify-between text-[11px] text-muted-ink border-b border-slate-100 pb-2">
                                    <span>PRC Weight: <strong
                                            class="text-slate-700">{{ number_format($domain['weight'] ?? 0, 1) }}%</strong></span>
                                    <span>Contribution: <strong
                                            class="text-slate-900">+{{ number_format($domain['contribution'] ?? 0, 2) }}%</strong></span>
                                </div>
                            </div>
            ...
            <div class="rounded-xl border border-slate-200 bg-slate-50/70 p-4 text-xs text-muted-ink leading-relaxed">
                ...
                Each domain contribution is calculated as (Domain Mastery × PRC Weight) ÷ Total PRC Weight × 100.
                Readiness bands are internal CitiRx guidance categories designed to steer adaptive study paths and do
                not represent official PRC licensure determinations.
            </div>
            ...
            <div class="flex flex-col sm:flex-row items-center justify-center gap-3 pt-2">
                ...
                <a href="{{ route('dashboard') }}" style="--lip: #CBD5E1;"
