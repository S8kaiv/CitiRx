...
class QuestionSeeder extends Seeder
{
    ...
    {
        // 2 questions per competency, keyed by competency_code
        $questions = [
            'PC-01' => [
                [
        ...
        foreach ($questions as $code => $items) {
            $competency = TosCompetency::where('competency_code', $code)->first();

            if (! $competency) {
                $this->command->warn("⚠ Skipping {$code} — competency not found. Run TosSeeder first.");
                continue;
            }

            foreach ($items as $item) {
                // Use a composite natural key: question_text + competency_id
                // Idempotent — running twice won't duplicate questions
        ...
        $this->command->info("✓ Question seeder: {$questionsProcessed} questions, {$choicesProcessed} choices.");
    }
}